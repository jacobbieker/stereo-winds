...
def _materialization_metadata(
    ...
    return {
        ...
        "status": MetadataValue.text("reused existing output" if result.reused else "computed"),
    }
...
def build_amv_asset(
    ...
    *,
    ...
    def _amv_asset(
        ...
        reuse_expected = (
            run_settings.skip_existing and _existing_output(paths.output_dir, sat_id, t0).exists()
        )
        if reuse_expected:
...
def build_amv_assets(
    ...
    **kwargs: Any,
    ...
    return {sat_id: build_amv_asset(sat_id, partitions_def, **kwargs) for sat_id in satellites}
