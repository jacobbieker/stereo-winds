...
def _scene(sat_id, zenith=10.0, u=1.0, n=16, lat=(-10.0, 10.0), lon=(-10.0, 10.0), t0=T0, **attrs):
    """A synthetic scene with a recognisable ``u_wind`` and viewing angle."""
    ds = synthetic_scene(sat_id, t0, ny=n, nx=n, zenith=zenith, lat_range=lat, lon_range=lon)
...
class TestBuildMosaic:
    def test_two_satellites_produce_a_populated_mosaic(self):
        per_sat = {"goes18": _scene("goes18", zenith=20.0), "goes19": _scene("goes19", zenith=10.0)}
    ...
    def test_disjoint_satellites_both_appear(self):
        per_sat = {
            "goes19": _scene("goes19", u=19.0, lon=(-40.0, -20.0)),
...
class TestMissingSatellites:
    def test_absent_ids_are_reported_in_expected_order(self):
        assert missing_satellites({"goes19": None}, ["goes18", "goes19", "gk2a"]) == [
            "goes18",
            "gk2a",
        ]
...
class TestPartialMosaic:
    def test_a_missing_satellite_still_yields_a_mosaic(self):
        ...
        per_sat = {
            "goes18": _scene("goes18", lon=(-150.0, -130.0)),
            "goes19": _scene("goes19", lon=(-85.0, -65.0)),
        }
        ds = build_mosaic(
            per_sat, T0, resolution_m=RES, expected=["goes18", "goes19", "himawari9", "gk2a"]
        )
    ...
    def test_all_satellites_empty_raises_rather_than_an_all_nan_grid(self):
        ...
        with pytest.raises(EmptyMosaicError, match="goes18, goes19.*usable grid cell"):
            build_mosaic(per_sat, T0, resolution_m=RES)
...
class TestWriteMosaicNetcdf:
    ...
    def test_round_trips_through_xarray(self, tmp_path):
        ...
        ds = build_mosaic(per_sat, T0, resolution_m=RES, expected=["goes18", "goes19", "gk2a"])
        path = write_mosaic_netcdf(ds, tmp_path, T0)
        ...
        with xr.open_dataset(path) as back:
            ...
            np.testing.assert_array_equal(
                back["u_wind"].values[filled], ds["u_wind"].values[filled]
            )
    ...
    def test_overwrites_an_existing_mosaic_atomically(self, tmp_path):
        first = build_mosaic({"goes18": _scene("goes18", u=18.0)}, T0, resolution_m=RES)
        path = write_mosaic_netcdf(first, tmp_path, T0)
        second = build_mosaic({"goes19": _scene("goes19", u=19.0)}, T0, resolution_m=RES)
    ...
    def test_accepts_a_string_output_dir(self, tmp_path):
        ds = build_mosaic({"goes19": _scene("goes19")}, T0, resolution_m=RES)
        assert write_mosaic_netcdf(ds, str(tmp_path), T0) == (global_nc_path(Path(tmp_path), T0))
