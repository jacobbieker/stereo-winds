...
def _fake_infer(calls: list[tuple[str, datetime]]):
    ...
    def infer_satellite(sat_id, t0, model, disp, flow_bands, rad_bands,
...
def _partition_keys(partitions_def, n: int) -> list[str]:
    ...
    assert len(keys) > n, (
        f"The partitions definition offers only {len(keys)} key(s); "
        f"the test needs {n + 1}")
    ...
    try:
        ...
        resources = {
            ...
            "store": IcechunkStoreResource(
                store_uri=store_uri, branch="main", chunk=1024,
            ),
        ...
        commit_counts = []
        for key in keys:
            result = materialize(assets, partition_key=key,
                                 resources=resources)
...
class TestFullPipeline:
    ...
    def test_every_satellite_was_retrieved_per_partition(self, pipeline_run):
        ...
        expected = {(sat, t)
                    for t in pipeline_run["times"] for sat in SATELLITES}
    ...
    def test_store_time_axis(self, pipeline_run):
        ...
        with _open_store(pipeline_run["store_uri"]) as ds:
            ...
            for i, t0 in enumerate(times):
                assert stored[i] == np.datetime64(t0, "ns"), (
    ...
    def test_store_variables(self, pipeline_run):
        ...
        with _open_store(pipeline_run["store_uri"]) as ds:
            ...
            assert source.dtype == np.int8, (
                f"source_satellite_index is {source.dtype}, not int8")
    ...
    def test_one_commit_per_timestep(self, pipeline_run):
        ...
        for count in counts:
            assert count == previous + 1, (
                f"commit count went {previous} -> {count}; expected one "
                f"commit per timestep")
            previous = count
    ...
    def test_mosaic_values_follow_the_scenes(self, pipeline_run):
        ...
        with _open_store(pipeline_run["store_uri"]) as ds:
            ...
            assert found <= {np.float32(v) for v in expected}, (
            ...
            for sat in ("gk2a", "himawari9"):
                assert abs(OVERLAP_LON - SUB_LON_DEG[sat]) < HALF_WIDTH_DEG, (
                    f"{sat} does not cover the supposed overlap point")

            names = str(
                ds["source_satellite_index"].attrs["flag_meanings"]).split()
            source = first["source_satellite_index"].values
            won = {name: int(np.count_nonzero(source == code))
            ...
            cell = first.sel(latitude=OVERLAP_LAT, longitude=OVERLAP_LON,
                             method="nearest")
    ...
    def test_source_codes_mean_the_same_satellite_at_every_timestep(
            self, pipeline_run):
        ...
        with _open_store(pipeline_run["store_uri"]) as ds:
            ...
            for step in range(ds.sizes["time"]):
                cell = ds.isel(time=step).sel(
                    latitude=OVERLAP_LAT, longitude=OVERLAP_LON,
                    method="nearest")
                assert names[int(cell["source_satellite_index"])] == "gk2a"
