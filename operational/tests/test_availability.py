...
def _every(
    minutes: int, n: int, skip: tuple[int, ...] = (), offset_seconds: int = 0, base: datetime = T0
) -> np.ndarray:
...
def stub_availability(monkeypatch):
    ...
    def fake(sat_id, band, start, end, product="ABI-L1b-RadF", include_s3_fallback=True):
        table.calls.append((sat_id, band, start, end, product, include_s3_fallback))
...
class TestProbeSatellite:
    def test_reports_band_and_scan_interval(self, stub_availability):
        ...
        avail = probe_satellite("goes19", T0, T0 + timedelta(hours=1), ["C14"], ["C14"])
    ...
    def test_window_is_padded_for_neighbour_frames(self, stub_availability):
        stub_availability["goes19"] = _every(10, 6)
        probe_satellite(
            "goes19", T0, T0 + timedelta(hours=1), ["C14"], ["C14"], tolerance_minutes=5.0
        )
    ...
    def test_unsorted_scan_times_are_sorted(self, stub_availability):
        shuffled = np.array(
            [np.datetime64(T0 + timedelta(minutes=10 * i), "ns") for i in (3, 0, 2, 1)],
            dtype="datetime64[ns]",
        )
        stub_availability["goes19"] = shuffled
        avail = probe_satellite("goes19", T0, T0 + timedelta(minutes=30), ["C14"], ["C14"])
    ...
    def test_hand_built_instance_sorts_its_scan_times(self):
        avail = SatelliteAvailability(
            ...
            band="C14",
            scan_times=np.array(
                [np.datetime64(T0 + timedelta(minutes=10 * i), "ns") for i in (2, 0, 1)],
                dtype="datetime64[ns]",
            ),
...
class TestDistinctFrames:
    ...
    def test_one_scan_cannot_serve_two_frames(self):
        ...
        # retrievable triplet.
        avail = self._avail([T0, T0 + timedelta(minutes=20)], tolerance_minutes=11.0)
        assert avail.can_deliver(T0 + timedelta(minutes=10)) is False

    def test_three_real_scans_are_accepted(self):
        avail = self._avail(
            [T0, T0 + timedelta(minutes=10), T0 + timedelta(minutes=20)], tolerance_minutes=11.0
        )
...
class TestAvailableTimestamps:
    def test_full_triplet_is_included(self, stub_availability):
        ...
        got = available_timestamps("goes19", T0, T0 + timedelta(minutes=40), ["C14"], ["C14"])
    ...
    def test_missing_neighbour_frame_is_excluded(self, stub_availability):
        ...
        got = available_timestamps("goes19", T0, T0 + timedelta(minutes=50), ["C14"], ["C14"])
    ...
    def test_candidates_are_limited_to_the_window(self, stub_availability):
        ...
        got = available_timestamps(
            "goes19", T0 + timedelta(minutes=30), T0 + timedelta(minutes=50), ["C14"], ["C14"]
        )
        assert got == [
            T0 + timedelta(minutes=30),
    ...
    def test_no_scans_gives_nothing(self, stub_availability):
        ...
        assert available_timestamps("goes19", T0, T0 + timedelta(hours=1), ["C14"], ["C14"]) == []

    def test_reversed_window_is_empty(self, stub_availability):
        stub_availability["goes19"] = _every(10, 6)
        assert available_timestamps("goes19", T0 + timedelta(hours=1), T0, ["C14"], ["C14"]) == []
...
class TestTolerance:
    ...
    def test_tolerance_is_inclusive_at_the_edge(self):
        ...
        assert (
            SatelliteAvailability("goes19", "C14", _t64(*scans), 10, 5.0).can_deliver(slot) is True
        )
        assert (
            SatelliteAvailability("goes19", "C14", _t64(*scans), 10, 4.99).can_deliver(slot)
            is False
        )
    ...
    def test_equidistant_scans_are_ambiguous(self):
        ...
        avail = SatelliteAvailability(
            "goes19",
            "C14",
            _t64(*[T0 + timedelta(minutes=10 * i, seconds=300) for i in range(7)]),
            10,
            5.0,
        )
        assert avail.can_deliver(T0 + timedelta(minutes=20)) is False

    def test_offset_cadence_over_a_10_minute_satellite_is_rejected(self, stub_availability):
...
class TestNewTimestamps:
    ...
    def test_since_at_or_after_until_is_empty(self, stub_availability):
        ...
        assert new_timestamps("goes19", T0, T0, ["C14"], ["C14"], cadence_minutes=10) == []
        assert stub_availability.calls == []
    ...
    def test_stale_cursor_is_capped_by_the_lookback(self, stub_availability, caplog):
        ...
        stub_availability["goes19"] = _t64(*[until - timedelta(minutes=10 * i) for i in range(20)])
        with caplog.at_level("WARNING"):
            got = new_timestamps(
                "goes19", T0, until, ["C14"], ["C14"], cadence_minutes=60, lookback_hours=2.0
            )
    ...
    def test_recent_cursor_is_not_capped(self, stub_availability):
        ...
        assert got == [T0 + timedelta(hours=1), T0 + timedelta(hours=2), T0 + timedelta(hours=3)]

    def test_hourly_cadence_lands_on_the_hour(self, stub_availability):
        base = datetime(2026, 8, 1, 3, 17)
        stub_availability["goes19"] = _t64(*[base + timedelta(minutes=10 * i) for i in range(30)])
        got = new_timestamps(
            "goes19", base, base + timedelta(hours=3), ["C14"], ["C14"], cadence_minutes=60
        )
        assert got == [
    ...
    def test_fifteen_minute_satellite_needs_a_15_minute_triplet(self, stub_availability):
...
class TestReadySatellites:
    def test_reports_one_entry_per_satellite(self, stub_availability):
        ...
        ready = ready_satellites(
            T0 + timedelta(minutes=20), ["goes19", "gk2a", "himawari9"], ["C14"], ["C14"]
        )
    ...
    def test_missing_neighbour_makes_a_satellite_unready(self, stub_availability):
        ...
        stub_availability["goes19"] = _every(10, 7)
        ready = ready_satellites(T0 + timedelta(minutes=20), ["goes18", "goes19"], ["C14"], ["C14"])
...
class TestSatelliteWithoutRequestedBand:
    ...
    def test_probe_returns_band_none_without_a_lookup(self, stub_availability):
        avail = probe_satellite("mtg-i1", T0, T0 + timedelta(hours=1), ["C14"], ["C14"])
    ...
    def test_available_timestamps_is_empty(self, stub_availability):
        assert available_timestamps("mtg-i1", T0, T0 + timedelta(hours=1), ["C14"], ["C14"]) == []
    ...
    def test_ready_satellites_marks_it_false(self, stub_availability):
        stub_availability["goes19"] = _every(10, 7)
        ready = ready_satellites(T0 + timedelta(minutes=20), ["goes19", "mtg-i1"], ["C14"], ["C14"])
        assert ready == {"goes19": True, "mtg-i1": False}

    def test_empty_band_lists_are_treated_as_nothing_available(self, stub_availability):
        avail = probe_satellite("goes19", T0, T0, [], [])
        assert avail.band is None
        assert available_timestamps("goes19", T0, T0 + timedelta(hours=1), [], []) == []
...
class TestCadenceGrid:
    def test_slots_are_anchored_to_the_epoch(self):
        got = cadence_grid(datetime(2026, 8, 1, 0, 5), datetime(2026, 8, 1, 2, 30), 60)
        assert got == [datetime(2026, 8, 1, 1, 0), datetime(2026, 8, 1, 2, 0)]

    def test_bounds_are_inclusive(self):
        got = cadence_grid(datetime(2026, 8, 1, 1, 0), datetime(2026, 8, 1, 2, 0), 60)
        assert got == [datetime(2026, 8, 1, 1, 0), datetime(2026, 8, 1, 2, 0)]

    def test_reversed_window_is_empty(self):
        assert cadence_grid(datetime(2026, 8, 1, 2, 0), datetime(2026, 8, 1, 1, 0), 60) == []

    def test_ten_minute_grid(self):
        got = cadence_grid(T0, T0 + timedelta(minutes=25), 10)
        assert got == [T0, T0 + timedelta(minutes=10), T0 + timedelta(minutes=20)]
